...
- Supabase CLI configuration, nine ordered migrations, deterministic catalog seed, RLS isolation tests, atomic profile/item replacement RPCs, and transactional session lifecycle RPCs are now present. A disposable local PostgreSQL run passes them; Docker-backed `supabase db reset` remains to be run when Docker is available.
