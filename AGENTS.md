...
- Supabase CLI configuration, eight ordered migrations, deterministic catalog seed, RLS isolation tests, the atomic profile replacement RPC, and transactional session lifecycle RPCs are now present. A disposable local PostgreSQL run passes them; Docker-backed `supabase db reset` remains to be run when Docker is available.
